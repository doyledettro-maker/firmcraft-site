...
## Implementation Status (as of June 14, 2026)
...
| **2.4** — Mobile App MVP | 🟡 Built, in device testing (June 14) | Implemented in the `firmcraft-mobile` repo (Expo SDK 56 / RN 0.85 New Arch, expo-router, PowerSync offline-first via `@powersync/op-sqlite`). Shipped: employee-code sign-in (Clerk `ticket` strategy) + Face ID unlock, daily schedule with day-strip, job detail (checklist / tech-notes / parts-used / photo grid / equipment / status-history), one-tap status progression, 4-step completion wizard (photos → notes/parts → customer signature → confirm), adaptive GPS location sharing → `tech-location` Edge Function, push-token registration, offline write queue + flush-on-reconnect. Distributed via EAS **internal preview** builds (not yet TestFlight/App Store). **Clerk→RLS claims bridge now live (addresses the Sprint-3 / CRIT-1 deferral):** the `supabase` Clerk JWT template emits `tenant_id` / `tech_id` / `app_role` from `user.public_metadata`; demo-tenant testers Doyle (`1001`) + Robert (`1002`) are provisioned as technicians with claims + a seeded multi-status workload. Native API enabled on the prod Clerk instance (was the cause of an init hang). **Still to do:** gate the dispatch board's service-role API routes behind the same JWT bridge; in-app map / "where's my tech"; end-to-end notification deep-links (Phase 2.5). |
| **2.5** — Notifications + Polish | 🟡 Partial | Mobile already registers push tokens (`device_tokens`) and routes notification taps to `/jobs/{id}`; the server-side delivery worker + SMS confirmations/reminders/ETA updates are not built. |
