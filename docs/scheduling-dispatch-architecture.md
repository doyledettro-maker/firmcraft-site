...
8. **Phase 2.1 Sprint 3 landed June 10 (out of order, after 2.2/2.3).** The 8 job-management/availability skills from the §4 catalog are built in `hermes-skills/scheduling/` (zero-dependency Node modules; Edge Function calls with direct-PostgREST fallback until the functions deploy). Storage buckets `job-photos`/`signatures` exist with tenant-scoped path RLS (migration `20260610_014`), and the §9.4 webhook queue exists as `webhook_events` (19th table) + jobs lifecycle triggers (migration `20260610_013`) — **queue only; the delivery worker (tenant URLs, retries) is Phase 2.5**. Still not built: slot-hold tokens, the §4 dispatch/tech-management/communication/configuration skill groups, and installation of the skills into the dogfood Hermes registry.
